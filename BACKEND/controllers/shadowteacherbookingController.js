const ShadowTeacherBooking = require('../models/shadowteacherbooking');
...
exports.addBooking = async(req, res) => {
    ...
    const schoolTimeDuration = req.body.schoolTimeDuration;
    const childNeeds = req.body.childNeeds;
    const startingDate = req.body.startingDate;

        const newBooking = ShadowTeacherBooking({
            childName,
            childAge,
            schoolGrade,
            requirementsNeeds,
...
exports.viewBooking = async(req, res) => {
    try{
        const bookings = await ShadowTeacherBooking.find();
...
exports.updateBooking = async(req, res) => {
    ...
    const { childName, childAge, schoolName, location, schoolGrade, 
        schoolTimeDuration, requirementsNeeds, childNeeds, diagnosedCondition, startingDate, status} = req.body;
    
    const updatedBooking = { childName, childAge, schoolName, location, schoolGrade, 
        schoolTimeDuration, requirementsNeeds, childNeeds, diagnosedCondition, startingDate, status};

    
    try {
        await ShadowTeacherBooking.findByIdAndUpdate(bookingId, updatedBooking);
...
exports.deleteBooking = async (req, res) => {
    ...
    try {
        await ShadowTeacherBooking.findByIdAndDelete(bookingId);
...
exports.getBooking = async(req, res) => {
    ...
    try {
        const bookingteacher = await ShadowTeacherBooking.findById(bookingId);
